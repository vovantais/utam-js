...
import UtamTestComponent from '../../pageObjects/utamTestComponent'
...
describe('Scratch Org Tests', () => {
    ...
    it('should display record', async () => {
        console.log("Click 'UtamTest' in app navigation menu and wait for URL navigation");
        const appNav = await appHomePage.getNavigationBar();
        const appNavBar = await appNav.getAppNavBar();
        const item = await appNavBar.getNavItem('UtamTest');
        await item.clickAndWaitForUrl('lightning/n/UtamTest');

        console.log('Wait for Flexipage with UtamTest component to load');
        const component2 = await appHomePage.getComponent();
        const getData = await component2.getContent(UtamTestComponent);

        const text = await getData.getContent();
